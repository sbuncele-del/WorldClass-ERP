...
export async function runManufacturingMigration(pool: Pool): Promise<void> {
  ...
  try {
    ...
    // Indexes
    await client.query(`CREATE INDEX IF NOT EXISTS idx_bom_item ON manufacturing.bill_of_materials(item_id);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_prod_order_status ON manufacturing.production_orders(status);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_mrp_results_item ON manufacturing.mrp_results(item_id);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_work_center_code ON manufacturing.work_centers(work_center_code);`);

    await client.query('COMMIT');
    console.log('✅ Manufacturing Module migration completed successfully.');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('❌ Error running Manufacturing Module migration:', error);
    throw error;
  } finally {
    client.release();
  }
