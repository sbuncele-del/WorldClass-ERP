...
BEGIN
    ...
    IF NEW.asset_status = 'ACTIVE' AND OLD.asset_status = 'DRAFT' AND NEW.journal_entry_id IS NULL THEN
        ...
        v_description := 'IAS 16 - Initial Recognition: ' || NEW.asset_name || ' (' || NEW.asset_number || ')';
        ...
        RAISE NOTICE 'IAS 16 - Asset acquisition posted: %, Amount %', 
            NEW.asset_number, NEW.purchase_price;
    END IF;
...
BEGIN
    ...
    IF NEW.asset_status = 'CALCULATED' AND NEW.posted_to_gl = false THEN
        ...
        v_description := 'IAS 16.50 - Depreciation: ' || v_asset_name || ' (' || v_asset_number || ') - ' || 
        ...
        NEW.posted_to_gl := true;
        NEW.posted_at := CURRENT_TIMESTAMP;
        NEW.asset_status := 'POSTED';
        
        RAISE NOTICE 'IAS 16 - Depreciation posted: %, Amount %', v_asset_number, NEW.depreciation_amount;
    END IF;
...
BEGIN
    ...
    IF NEW.approved_by IS NOT NULL AND NEW.posted_to_gl = false THEN
        ...
        -- Description
        v_description := 'IAS 16.31 - Revaluation: ' || v_asset_name || ' (' || v_asset_number || ') - ' ||
        ...
        RAISE NOTICE 'IAS 16 - Revaluation posted: %, Surplus/Deficit: %', 
            v_asset_number, NEW.revaluation_surplus_deficit;
    END IF;
...
BEGIN
    ...
    IF NEW.asset_status = 'COMPLETED' AND OLD.asset_status IN ('SCHEDULED', 'IN_PROGRESS') 
       AND NEW.journal_entry_id IS NULL THEN
        
        -- Get asset details
        SELECT asset_number, asset_name
        INTO v_asset_number, v_asset_name
        FROM fixed_assets
        WHERE asset_id = NEW.asset_id;
        ...
        IF v_capitalize THEN
            v_description := 'IAS 16.12 - Capitalize Subsequent Cost: ' || v_asset_name || 
                           ' (' || v_asset_number || ') - ' || NEW.description;
        ELSE
        ...
        RAISE NOTICE 'IAS 16 - Maintenance posted: %, Cost: %, Capitalized: %', 
            v_asset_number, NEW.total_cost, v_capitalize;
    END IF;
...
BEGIN
    ...
    IF NEW.approved_by IS NOT NULL AND NEW.posted_to_gl = false THEN
        ...
        -- Get asset details
        SELECT 
            asset_number, asset_name, purchase_price, accumulated_depreciation
        INTO 
            v_asset_number, v_asset_name, v_asset_cost, v_accumulated_dep
        FROM fixed_assets
        WHERE asset_id = NEW.asset_id;
        ...
        -- Description
        v_description := 'IAS 16.67 - Asset Disposal: ' || v_asset_name || ' (' || v_asset_number || ') - ' ||
        ...
        RAISE NOTICE 'IAS 16 - Asset disposal posted: %, Gain/Loss: %', v_asset_number, NEW.gain_loss;
    END IF;
...
BEGIN
    ...
    IF NEW.approved_by IS NOT NULL AND NEW.posted_to_gl = false THEN
        ...
        -- Description
        v_description := 'IAS 36 - Impairment Loss: ' || v_asset_name || ' (' || v_asset_number || ') - ' ||
        ...
        RAISE NOTICE 'IAS 36 - Impairment posted: %, Loss: %', v_asset_number, NEW.impairment_loss;
    END IF;
