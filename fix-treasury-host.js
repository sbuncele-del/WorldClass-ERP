/**
 * Fix treasury controller on host - replace broken getTreasuryDashboard
 */
const fs = require('fs');
const file = '/tmp/treasury.controller.v2.js';
...
if (exportsIdx === -1) {
  // Maybe the line got mangled
  const altExport = code.indexOf('exports.getTreasuryDashboard', funcStart);
  if (altExport === -1) {
  ...
  const oldExport = code.substring(altExport, eol > -1 ? eol : altExport + 100);
...
`;
  code = code.substring(0, funcStart) + replacement + oldExport + code.substring(eol > -1 ? eol : altExport + 100);
} else {
  const replacement = `async function getTreasuryDashboard(req, res) {
    try {
        const { tenantId } = getTenantContext(req);
