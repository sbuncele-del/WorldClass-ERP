...
INSERT INTO fixed_assets (
    ...
    depreciation_start_date, asset_status, created_by
) 
SELECT 
...
UPDATE fixed_assets SET asset_status = 'ACTIVE'
WHERE asset_number = 'MCH-2025-001' AND asset_status = 'DRAFT';
...
JOIN gl_transactions glt ON je.entry_id = glt.journal_entry_id
JOIN chart_of_accounts coa ON glt.account_code = coa.account_code
WHERE fa.asset_number = 'MCH-2025-001'
...
WHERE asset_id = (SELECT asset_id FROM fixed_assets WHERE asset_number = 'MCH-2025-001')
...
JOIN gl_transactions glt ON je.entry_id = glt.journal_entry_id
JOIN chart_of_accounts coa ON glt.account_code = coa.account_code
WHERE ads.asset_id = (SELECT asset_id FROM fixed_assets WHERE asset_number = 'MCH-2025-001')
...
SELECT asset_number, asset_name, purchase_price as cost, 
       accumulated_depreciation, net_book_value
FROM fixed_assets
WHERE asset_number = 'MCH-2025-001';
...
SELECT asset_number, asset_name, purchase_price as revalued_amount,
       accumulated_depreciation, net_book_value,
...
SELECT asset_number, asset_name, purchase_price, accumulated_depreciation,
       net_book_value
FROM fixed_assets
WHERE asset_number = 'MCH-2025-001';
