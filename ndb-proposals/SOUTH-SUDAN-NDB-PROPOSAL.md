# National Development Bank of South Sudan
## Investment & Development Finance Business Plan

**Presented to:** Strategic Investment Partners  
**Prepared by:** Alex Tsela Global (ATG) Holdings  
**Date:** January 2026  
**Document Classification:** Confidential — Investment Memorandum  
**Version:** 1.0



## Table of Contents

| Section | Title | Page |
|:-------:|-------|:----:|
| 1 | Executive Summary | 3 |
| 2 | Institutional Overview | 5 |
| 3 | Market Opportunity | 6 |
| 4 | Business Model | 8 |
| 5 | Strategic Sectors | 10 |
| 6 | Funding Sources Strategy | 13 |
| 7 | Organizational Structure | 16 |
| 8 | Financial Projections | 18 |
| 9 | Risk Analysis & Mitigation | 20 |
| 10 | Implementation Roadmap | 21 |
| 11 | Comparable Institutions | 23 |
| 12 | Investment Ask & Terms | 24 |
| 13 | Conclusion | 25 |
| 14 | Appendices | 26 |

<div style="page-break-before: always;"></div>

## Executive Summary

This business plan presents the establishment of the **National Development Bank of South Sudan (NDB-SS)**, a specialized development finance institution designed to mobilize investment capital for strategic national infrastructure and economic development projects.

Modeled after successful development finance institutions such as the **Development Bank of Southern Africa (DBSA)**, **African Development Bank (AfDB)**, and **Qatar Development Bank**, NDB-SS will serve as the primary vehicle for channeling both commercial investment and donor funding into transformative projects across South Sudan and the broader African region.

### Seed Capital Request

**We are seeking USD 10,000,000 (Ten Million US Dollars) in seed capital** to establish and operationalize the National Development Bank of South Sudan. This seed investment will fund:

- Legal establishment and regulatory licensing
- Initial operations and working capital (12 months)
- Technology infrastructure and banking systems
- Senior management team recruitment
- First project pipeline development

**This seed capital will enable NDB-SS to mobilize USD 1.2 Billion over 3 years** (USD 400 Million annually) through deal origination, earning approximately **USD 20 Million in commission revenue per year**.

### Investment Highlights

| Metric | Value |
|--------|-------|
| **Seed Capital Required** | **USD 5,000,000** |
| **3-Year Capital Mobilization Target** | **USD 1,200,000,000** |
| **Annual Capital Mobilization** | **USD 400,000,000** |
| **Primary Sectors** | Roads, Mining, Agro-processing |
| **Geographic Focus** | South Sudan, East Africa |
| **Revenue Model** | Commission-based (5%) + Transaction Fees |
| **Projected Year 1 Revenue** | USD 24,000,000 |
| **Projected Year 1 Net Profit** | USD 18,000,000 |
| **Year 1 Return on Seed Investment** | **360%** |

### Strategic Value Proposition

1. **Critical Infrastructure Gap**: South Sudan requires an estimated $50+ billion in infrastructure investment
2. **Development Finance Deficit**: No dedicated national development bank currently exists
3. **Donor Coordination**: Fragmented humanitarian funding needs unified channel
4. **Economic Diversification**: Oil-dependent economy needs infrastructure for growth
5. **Regional Integration**: East African connectivity requires coordinated investment



<div style="page-break-before: always;"></div>

## 1. Institutional Overview

### 1.1 Vision

To be the premier development finance institution driving sustainable economic transformation in South Sudan and contributing to Africa's infrastructure development agenda.

### 1.2 Mission

To mobilize, structure, and deploy development capital for strategic national projects that create jobs, build infrastructure, and foster sustainable economic growth while ensuring financial viability and accountability.

### 1.3 Core Values

| Value | Description |
|-------|-------------|
| **Integrity** | Transparent governance and ethical operations |
| **Impact** | Measurable developmental outcomes |
| **Sustainability** | Financially viable projects with long-term benefits |
| **Partnership** | Collaborative approach with governments, donors, and investors |
| **Innovation** | Creative financing solutions for complex challenges |

### 1.4 Legal Structure

| Element | Specification |
|---------|---------------|
| Entity Type | Development Finance Institution (DFI) |
| Jurisdiction | Republic of South Sudan |
| Regulatory Framework | Central Bank of South Sudan + Special Charter |
| Ownership Structure | Government + Strategic Partners + Private Investors |
| Governance | Independent Board with international representation |



<div style="page-break-before: always;"></div>

## 2. Market Opportunity

### 2.1 South Sudan Development Context

South Sudan, the world's youngest nation (independence 2011), faces enormous development challenges but also unprecedented opportunities for transformative investment.

**Key Economic Indicators:**

| Indicator | Value |
|-----------|-------|
| Population | ~12 million |
| GDP | ~$4.5 billion |
| GDP per Capita | ~$375 |
| Economic Dependence | 95%+ on oil exports |
| Infrastructure Gap | $50+ billion estimated |
| Road Network | <200km paved (entire country) |
| Electrification Rate | <5% |
| Banking Penetration | <10% |

### 2.2 Infrastructure Deficit

**Roads & Transport:**
- Less than 200km of paved roads in a country larger than France
- No functioning railway network
- Limited air transport infrastructure
- River transport underutilized despite extensive Nile system

**Energy:**
- Less than 5% of population has electricity access
- Massive hydropower potential (Nile) untapped
- Solar potential among highest in Africa

**Agriculture:**
- Only 4% of arable land currently cultivated
- Massive irrigation potential from Nile basin
- Food imports despite agricultural potential

### 2.3 Funding Landscape

**Current Development Finance Sources:**

| Source | Annual Flow (Est.) | Limitations |
|--------|-------------------|-------------|
| Humanitarian Aid | $1.5-2 billion | Short-term, fragmented |
| Bilateral Aid | $500 million | Tied to donor priorities |
| Multilateral (World Bank, AfDB) | $300 million | Slow disbursement |
| Private Investment | <$100 million | High risk perception |
| Oil Revenue (Government) | Variable | Volatile, misallocated |

**Gap Analysis:**
- **Required Annual Investment**: $5-10 billion for meaningful development
- **Current Investment**: <$3 billion (mostly humanitarian)
- **Funding Gap**: $2-7 billion annually
- **Development Finance Gap**: No dedicated DFI to structure bankable projects

### 2.4 Regional Opportunity

**East African Community Integration:**
- South Sudan seeking EAC membership
- Regional infrastructure connectivity needed
- Cross-border trade potential with Uganda, Kenya, Ethiopia, DRC

**Pan-African Development Agenda:**
- African Continental Free Trade Area (AfCFTA)
- Programme for Infrastructure Development in Africa (PIDA)
- Agenda 2063



<div style="page-break-before: always;"></div>

## 3. Business Model

### 3.1 Dual-Track Approach

NDB-SS operates two complementary business streams:

#### Track A: Commercial Development Finance
- Source investment funds for viable, revenue-generating projects
- Structure deals to ensure repayment
- Focus: Infrastructure with commercial returns (toll roads, power, mining)

#### Track B: Donor Fund Coordination
- Attract and manage donor/humanitarian funding
- Channel to development-aligned humanitarian projects
- Focus: Social infrastructure (schools, hospitals, water systems)

### 3.2 Revenue Model

**Primary Revenue Streams:**

| Revenue Source | Description | Rate/Fee |
|----------------|-------------|----------|
| **Deal Origination Commission** | Fee for connecting funders to bankable projects | **5% of deal value** |
| **Fund Management Fee** | Annual fee for managing deployed capital | 1-2% AUM |
| **Transaction/Paymaster Fee** | Fee for disbursement management | 0.5-1% of disbursements |
| **Advisory Services** | Project preparation, feasibility studies | Project-based |
| **Success Fees** | Bonus on project completion/milestones | Negotiated |

**Example Revenue Calculation (Year 1):**

| Activity | Volume | Rate | Revenue |
|----------|--------|------|---------|
| Deals Closed | $400 million | 5% | $20,000,000 |
| Paymaster Services | $300 million | 0.75% | $2,250,000 |
| Advisory/Other | Various | - | $2,000,000 |
| **Year 1 Total Revenue** | | | **$24,250,000** |

### 3.3 Operational Model

**Model 1: Deal Connector (Commission Model)**
```
Funder (DFI/Investor) ──► NDB-SS ──► National Project
                            │
                       5% Commission
                     for locking deal
```

**Model 2: Paymaster (Transaction Model)**
```
Funder ──► NDB-SS Account ──► Project Disbursements
               │                      │
         Fund Management         Transaction Fees
              Fee                   (0.5-1%)
```

**Model 3: Co-Investment**
```
NDB-SS Capital + Partner Capital ──► Project
              │
        Returns + Carried Interest
```



<div style="page-break-before: always;"></div>

## 4. Strategic Sectors

### 4.1 Priority Sector: Roads & Transport Infrastructure

**Rationale:**
- Most critical infrastructure gap
- Enables all other economic activity
- High visibility, nation-building impact
- Clear revenue models (tolls, fuel levies)
- Strategic trade corridor connecting South Sudan to Uganda and East African markets

**Priority Project: Juba-Nimule Highway**

| Project | Est. Cost | Priority | Status |
|---------|-----------|----------|--------|
| Juba-Nimule Highway (Uganda border) | $400M | Critical | Pipeline |

**Project Overview:**
The Juba-Nimule Highway is a 192km road connecting South Sudan's capital to the Uganda border, representing the country's most vital trade and transport corridor. This project will:
- Reduce travel time from 8+ hours to under 3 hours
- Enable efficient import/export of goods
- Connect to the Northern Corridor (Mombasa-Kampala)
- Generate toll revenue for sustainability

| **Roads Subtotal** | **$400 million** | |

### 4.2 Priority Sector: Mining & Natural Resources

**Rationale:**
- Vast untapped gold deposits in Eastern Equatoria
- Diversification from oil dependence critical
- High-value export potential with established global markets
- Attracts serious international mining investors

**Priority Project: Gold Mining Development**

| Project | Est. Cost | Priority | Status |
|---------|-----------|----------|--------|
| Gold Mining Development (Kapoeta Region) | $300M | High | Pipeline |

**Project Overview:**
The Kapoeta gold mining region in Eastern Equatoria holds significant untapped gold reserves. This project encompasses:
- Exploration and geological surveys
- Mining infrastructure development
- Processing facilities
- Environmental compliance systems
- Community development programs

Estimated gold reserves and favorable geology make this a compelling investment opportunity for mining companies and development financiers.

| **Mining Subtotal** | **$300 million** | |

### 4.3 Priority Sector: Agro-processing

**Rationale:**
- Value addition to agricultural produce critical for economic development
- Reduces post-harvest losses (currently 40%+)
- Creates employment in processing and packaging
- Enables export of processed goods to regional markets
- Foundation for food security and import substitution

**Priority Project: Agro-processing Facilities**

| Project | Est. Cost | Priority | Status |
|---------|-----------|----------|--------|
| Agro-processing Facilities | $200M | High | Pipeline |

**Project Overview:**
Establishment of modern agro-processing facilities to add value to South Sudan's agricultural output:
- Grain milling and flour production
- Oilseed processing (sesame, groundnuts)
- Fruit and vegetable processing
- Meat processing and cold chain
- Packaging and quality control facilities

These facilities will be strategically located near agricultural production zones and transport corridors.

| **Agro-processing Subtotal** | **$200 million** | |

### 4.4 Three-Year Pipeline Summary

**Conservative Mobilization Target: $1.2 Billion over 3 Years**

| Sector | 3-Year Target | Year 1 | Year 2 | Year 3 |
|--------|---------------|--------|--------|--------|
| Roads (Juba-Nimule) | $400M | $150M | $150M | $100M |
| Mining (Gold) | $300M | $100M | $100M | $100M |
| Agro-processing | $200M | $50M | $75M | $75M |
| Other (Energy, Social) | $300M | $100M | $100M | $100M |
| **Total** | **$1.2B** | **$400M** | **$425M** | **$375M** |

**Annual Commission Revenue (5%):**

| Year | Capital Mobilized | Commission (5%) |
|------|-------------------|----------------|
| Year 1 | $400 million | $20 million |
| Year 2 | $425 million | $21.25 million |
| Year 3 | $375 million | $18.75 million |
| **3-Year Total** | **$1.2 billion** | **$60 million** |



<div style="page-break-before: always;"></div>

## 5. Funding Sources Strategy

### 5.1 Target Funder Categories

**Tier 1: Multilateral Development Finance Institutions**

| Institution | Potential | Focus Areas |
|-------------|-----------|-------------|
| African Development Bank (AfDB) | $300-500M | Infrastructure, Regional |
| World Bank / IFC | $200-400M | Multi-sector |
| Islamic Development Bank (IsDB) | $200-300M | Infrastructure, Social |
| Arab Fund for Economic Development | $100-200M | Infrastructure |
| OPEC Fund (OFID) | $100-150M | Energy, Agriculture |

**Tier 2: Bilateral Development Agencies**

| Country/Agency | Potential | Focus Areas |
|----------------|-----------|-------------|
| China (Exim Bank, CDB) | $500M+ | Infrastructure |
| Qatar Fund for Development | $200-300M | Multi-sector |
| UAE (Abu Dhabi Fund) | $150-200M | Infrastructure |
| Kuwait Fund | $100-150M | Infrastructure |
| Saudi Fund for Development | $100-150M | Social, Infrastructure |
| Japan (JICA) | $100-200M | Infrastructure |
| Germany (KfW) | $50-100M | Energy, SME |
| USA (DFC) | $100-200M | Private sector |

**Tier 3: Private Investors & Funds**

| Category | Potential | Approach |
|----------|-----------|----------|
| Infrastructure Funds | $200-400M | Co-investment |
| Mining Companies | $300-500M | Project finance |
| Sovereign Wealth Funds | $200-300M | Long-term debt |
| Impact Investors | $50-100M | Blended finance |

**Tier 4: Humanitarian & Donor Funding**

| Source | Potential | Focus |
|--------|-----------|-------|
| UN Agencies | $200-300M | Development-humanitarian |
| EU | $100-200M | Governance, social |
| USAID | $100-150M | Food security, health |
| UK FCDO | $50-100M | Multi-sector |

### 5.2 Funding Mobilization Strategy

**Phase 1 (Year 1): Foundation Building**
- Establish relationships with major DFIs
- Present pipeline of 3 priority bankable projects
- Secure anchor commitments ($100-150M)
- Target: $400 million mobilized

**Phase 2 (Year 2): Consolidation**
- Close additional transactions
- Build track record with successful disbursements
- Expand funder base
- Target: $425 million mobilized (cumulative $825M)

**Phase 3 (Year 3): Scale-Up**
- Proven track record attracts larger commitments
- Regional expansion consideration
- Target: $375 million mobilized (cumulative $1.2B)
- Evaluate commercial banking wing feasibility



<div style="page-break-before: always;"></div>

## 6. Organizational Structure

### 6.1 Governance Framework

```
┌─────────────────────────────────────────────────────────────┐
│                    BOARD OF DIRECTORS                       │
│   (Government + Strategic Partners + Independent Directors) │
│              Chairman + Non-Executive Directors             │
└─────────────────────────────────────────────────────────────┘
                              │
         ┌────────────────────┼────────────────────┐
         │                    │                    │
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│ Risk Committee  │  │ Audit Committee │  │Investment Comm. │
└─────────────────┘  └─────────────────┘  └─────────────────┘
                              │
┌─────────────────────────────────────────────────────────────┐
│                  MANAGING DIRECTOR / CEO                    │
└─────────────────────────────────────────────────────────────┘
                              │
         ┌────────────────────┼────────────────────┐
         │                    │                    │
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│      CFO        │  │  Development    │  │   Operations    │
│    Finance &    │  │    Finance      │  │   & Projects    │
│   Compliance    │  │                 │  │                 │
└─────────────────┘  └─────────────────┘  └─────────────────┘
```

### 6.2 Key Departments

**Development Finance Division**
- Deal origination and structuring
- Funder relationship management
- Project pipeline development
- Sector specialists (Roads, Mining, Agriculture)

**Operations & Projects Division**
- Project monitoring and supervision
- Disbursement management (Paymaster function)
- Technical assistance coordination
- Impact measurement

**Risk & Compliance Division**
- Credit risk assessment
- AML/KYC compliance
- Environmental & Social risk
- Portfolio monitoring

**Finance Division**
- Treasury management
- Financial reporting
- Budgeting and planning
- Investor relations

**Commercial Banking Division (Year 2)**
- Corporate banking services
- Trade finance
- Foreign exchange
- Treasury products

### 6.3 Staffing Plan (Lean Operations)

| Phase | Headcount | Key Positions |
|-------|-----------|---------------|
| Setup (Months 1-6) | 8 | MD/CEO, CFO, Head of Development Finance, Core support |
| Year 1 | 15 | Add Deal Origination, Operations, Compliance officers |
| Year 2 | 30 | Scale based on deal flow |
| Year 3+ | 45 | Gradual expansion tied to revenue |

**Lean Startup Approach:**
- Outsource non-core functions initially (legal, IT support, accounting)
- Use consultants for specialized deal structuring
- Focus hiring on revenue-generating roles
- Build capacity as deal flow grows



<div style="page-break-before: always;"></div>

## 7. Financial Projections

### 7.1 Five-Year Revenue Projection (Conservative)

| Revenue Stream | Year 1 | Year 2 | Year 3 | Year 4 | Year 5 |
|----------------|--------|--------|--------|--------|--------|
| Deal Commission (5%) | $20M | $21.25M | $18.75M | $30M | $50M |
| Fund Management Fees | $1M | $3M | $6M | $10M | $15M |
| Paymaster/Transaction | $2.25M | $3M | $4M | $6M | $8M |
| Advisory Services | $1M | $2M | $3M | $4M | $5M |
| Commercial Banking | - | - | $2M | $5M | $10M |
| **Total Revenue** | **$24.25M** | **$29.25M** | **$33.75M** | **$55M** | **$88M** |

*Note: Years 4-5 assume successful track record enables larger deal flow beyond initial $1.2B target*

### 7.2 Capital Deployment Trajectory (Conservative)

| Metric | Year 1 | Year 2 | Year 3 | Year 4 | Year 5 |
|--------|--------|--------|--------|--------|--------|
| Deals Closed | $400M | $425M | $375M | $600M | $1B |
| Cumulative Deployment | $400M | $825M | $1.2B | $1.8B | $2.8B |
| Active Projects | 3 | 5 | 6 | 10 | 15 |
| Jobs Created | 2,000 | 5,000 | 8,000 | 15,000 | 25,000 |

*Note: Years 4-5 projections assume successful execution of initial 3-year plan*

### 7.3 Operating Expense Projection (Lean Operations)

| Category | Year 1 | Year 2 | Year 3 | Year 4 | Year 5 |
|----------|--------|--------|--------|--------|--------|
| Personnel | $1.8M | $3M | $4.5M | $7M | $10M |
| Operations | $0.6M | $1M | $1.5M | $2.5M | $4M |
| Technology | $0.4M | $0.6M | $0.8M | $1.2M | $2M |
| Marketing/BD | $0.4M | $0.6M | $1M | $1.5M | $2M |
| Professional Fees | $0.8M | $1M | $1.2M | $1.8M | $2.5M |
| Other | $0.2M | $0.3M | $0.5M | $1M | $1.5M |
| **Total OpEx** | **$4.2M** | **$6.5M** | **$9.5M** | **$15M** | **$22M** |

### 7.4 Profitability Summary (Conservative)

| Metric | Year 1 | Year 2 | Year 3 | Year 4 | Year 5 |
|--------|--------|--------|--------|--------|--------|
| **Revenue** | $24.25M | $29.25M | $33.75M | $55M | $88M |
| **Operating Expenses** | $4.2M | $6.5M | $9.5M | $15M | $22M |
| **EBITDA** | **$20.05M** | **$22.75M** | **$24.25M** | **$40M** | **$66M** |
| **EBITDA Margin** | 83% | 78% | 72% | 73% | 75% |
| **Net Profit (Est.)** | **$18M** | **$20M** | **$21M** | **$35M** | **$58M** |

**3-Year Cumulative Performance:**
- Total Capital Mobilized: $1.2 billion
- Total Commission Revenue: $60 million
- Total Net Profit: $59 million



<div style="page-break-before: always;"></div>

## 8. Risk Analysis & Mitigation

### 8.1 Key Risks

| Risk Category | Risk | Likelihood | Impact | Mitigation |
|---------------|------|------------|--------|------------|
| **Political** | Instability, conflict | High | High | Diversify geography, donor partnerships |
| **Economic** | Oil price volatility | High | Medium | Focus on non-oil sectors |
| **Credit** | Project defaults | Medium | High | Rigorous due diligence, guarantees |
| **Operational** | Capacity constraints | Medium | Medium | International partnerships, training |
| **Regulatory** | Policy changes | Low | Medium | Government engagement, charter protection |
| **Reputational** | Project failures | Medium | High | Strong governance, impact monitoring |
| **FX** | Currency volatility | High | Medium | USD-denominated operations |

### 8.2 Risk Mitigation Framework

**Political Risk Mitigation:**
- Multilateral co-investment (MIGA, AfDB guarantees)
- Political risk insurance
- Diversification across regions (East Africa, Pan-African)
- Strong government relations with independence

**Credit Risk Mitigation:**
- Comprehensive due diligence
- Government guarantees where possible
- Collateral and security packages
- Phased disbursements tied to milestones

**Operational Risk Mitigation:**
- International advisory partnerships
- Robust internal controls
- Technology infrastructure
- Staff development programs



<div style="page-break-before: always;"></div>

## 9. Implementation Roadmap

### 9.1 Phase 1: Foundation (Months 1-6)

| Month | Activities |
|-------|------------|
| 1-2 | Legal establishment, charter negotiation |
| 2-3 | Board formation, senior recruitment |
| 3-4 | Office setup, systems implementation |
| 4-5 | Funder engagement, pipeline development |
| 5-6 | First deal structuring, operations launch |

**Key Milestones:**
- ✓ Legal entity established
- ✓ Board appointed
- ✓ CEO and key executives hired
- ✓ First 3 projects in pipeline
- ✓ First funder MOU signed

### 9.2 Phase 2: Operations Launch (Months 6-12)

| Month | Activities |
|-------|------------|
| 6-7 | First transaction closing |
| 7-8 | Paymaster operations begin |
| 8-10 | Build deal pipeline |
| 10-12 | Achieve Year 1 target |

**Key Milestones:**
- ✓ First $50M+ deal closed
- ✓ Paymaster function operational
- ✓ 2-3 funders active
- ✓ $400 million capital mobilized
- ✓ 3 priority projects funded

### 9.3 Phase 3: Commercial Wing (Year 2)

| Quarter | Activities |
|---------|------------|
| Q1 | Banking license application |
| Q2 | Commercial banking setup |
| Q3 | Product launch (corporate, trade finance) |
| Q4 | Scale commercial operations |



<div style="page-break-before: always;"></div>

## 10. Comparable Institutions

### 10.1 Development Bank of Southern Africa (DBSA)

| Metric | DBSA | NDB-SS Target |
|--------|------|---------------|
| Total Assets | $8 billion | $1.2 billion (3 years) |
| Annual Disbursements | $1.5 billion | $400 million (Year 1) |
| Staff | 500+ | 45 (Year 3) |
| Focus | Infrastructure | Infrastructure |
| Model | Direct lending + Advisory | Commission + Paymaster |

**Learnings from DBSA:**
- Strong government backing essential
- Infrastructure focus creates impact
- Development advisory adds value
- Regional expansion viable

### 10.2 Qatar Development Bank

| Metric | QDB | NDB-SS Target |
|--------|-----|---------------|
| Total Assets | $3 billion | $1.2 billion (3 years) |
| Focus | SME, Diversification | Infrastructure, Diversification |
| Revenue Model | Interest + Fees | Commission + Fees |

### 10.3 Rwanda Development Bank (BRD)

| Metric | BRD | NDB-SS Target |
|--------|-----|---------------|
| Portfolio | $400 million | $400 million (Year 1) |
| Sectors | Agriculture, Housing, SME | Roads, Mining, Agro-processing |
| Success Factor | Strong governance | Strong governance planned |



<div style="page-break-before: always;"></div>

## 11. Investment Ask

### 11.1 Seed Capital Requirements

| Category | Amount (USD) | Purpose |
|----------|--------------|---------|
| Working Capital | $2,500,000 | Operations, salaries (18 months lean) |
| Technology & Systems | $800,000 | Essential banking systems, IT |
| Office & Equipment | $400,000 | Juba HQ setup (modest) |
| Legal & Advisory | $1,000,000 | Structuring, licenses, compliance |
| Contingency | $300,000 | Unforeseen expenses |
| **Total Seed Capital** | **$5,000,000** | |

### 11.2 Return Profile (Conservative)

| Metric | Value |
|--------|-------|
| Seed Investment | $5 million |
| Year 1 Net Profit | $18 million |
| Year 1 ROI | **360%** |
| 3-Year Cumulative Profit | $59 million |
| 3-Year ROI | **1,180%** |
| 5-Year Cumulative Profit | $152 million |
| 5-Year ROI | **3,040%** |

### 11.3 Strategic Partnership Opportunity

We invite strategic partners to participate in the establishment of NDB-SS through:

1. **Equity Investment**: Founding shareholder status
2. **Technical Partnership**: DFI expertise, deal flow
3. **Anchor Commitment**: First major funding commitment
4. **Advisory Role**: Board or committee participation



<div style="page-break-before: always;"></div>

## 12. Conclusion

The **National Development Bank of South Sudan** represents a once-in-a-generation opportunity to establish a transformative development finance institution in one of Africa's most underserved markets.

### Why NDB-SS Will Succeed:

✅ **Massive Market Gap**: No DFI currently serves South Sudan's $50B+ infrastructure needs

✅ **Proven Model**: Commission-based revenue model de-risks operations

✅ **Focused Pipeline**: 3 priority projects ($900M) with clear development impact

✅ **Conservative Approach**: $1.2 billion target over 3 years is achievable and credible

✅ **Lean Operations**: Low overhead ensures profitability from Year 1

✅ **Development Impact**: Real contribution to nation-building and poverty reduction

✅ **Attractive Returns**: Strong ROI potential for founding partners (360% Year 1)

### Call to Action

We invite discussions with:
- **Development Finance Institutions** seeking deployment opportunities
- **Strategic Investors** interested in founding equity positions
- **Governments** seeking infrastructure investment channels
- **Humanitarian Donors** looking for development-aligned deployment

**Contact:**
Alex Tsela Global (ATG) Holdings  
Email: [To be provided]  
Phone: [To be provided]



## Appendices

### Appendix A: Project Pipeline Details
*Available upon request*

### Appendix B: Detailed Financial Model
*Available upon request*

### Appendix C: Legal Structure Documents
*Available upon request*

### Appendix D: Team Profiles
*Available upon request*



**Document Control**

| Field | Value |
|-------|-------|
| Document ID | NDB-SS-2026-001 |
| Version | 1.0 |
| Date | January 2026 |
| Classification | Confidential |
| Prepared by | ATG Holdings |



*© 2026 Alex Tsela Global (ATG) Holdings. All Rights Reserved.*

*This document contains confidential business information. Unauthorized distribution is prohibited.*
